...
def test_bill_of_entry_extracts_cleanly(
    extractor_registry: dict[str, DocumentExtractor],
) -> None:
    """`taxable_value`/`igst_amount` sit in a colon-less "Particular /
    Amount" table — see transaction_types.py's module docstring and
    `labelvalue.py`'s colon-less table-row matching for `money` fields,
    added in response to a follow-up ERP upload E2E report (2026-09-02)."""
    ...
    # Indian lakh/crore digit grouping, stripped to a plain decimal string —
    # not the Western 3-digit grouping a naive `\d{1,3}(,\d{3})*` would need.
    assert outcome.fields["taxable_value"] == "12450000"
    assert outcome.fields["igst_amount"] == "2405595"
...
async def test_bill_of_entry_promotes_a_real_transaction_document(
    tenant_a: SeededTenant,
    admin: psycopg.Connection[tuple[object, ...]],
    extractor_registry: dict[str, DocumentExtractor],
) -> None:
    """The table-row money grammar fix, taken all the way to a Silver write —
    not just proven at the extraction layer above."""
    extractor = extractor_registry["BILL_OF_ENTRY"]
    ...
    try:
        ...
        row = admin.execute(
            sql.SQL(
                "SELECT doc_type, direction, doc_number, total_taxable_value,"
                "       total_tax_value"
                "  FROM {}.transaction_document WHERE batch_id = %s"
            ).format(sql.Identifier(tenant_a.ctx.silver_schema)),
            (result.record_id,),
        ...
        assert row is not None
        assert row[0] == "BILL_OF_ENTRY"
        assert row[1] == "INWARD"
        assert row[2] == "6738214"
        assert row[3] == 12450000
        assert row[4] == 2405595
    finally:
        admin.execute(
            sql.SQL("DELETE FROM {}.transaction_document WHERE batch_id = %s").format(
                sql.Identifier(tenant_a.ctx.silver_schema)
